...
class Tracking_model extends CI_Model
 {
		...
		{
			$this->db->insert('tbl_shift',$formdata);	
			echo "1"; 	
		}
...
//-------------------------------------------------------------------------------------------------------
			 	...
			 	foreach ($location as  $value)
			 	 {
			 	 	 ...
			 	 	 $distance=$this->db->query(" 
											     SELECT IMEI,pos_date,pos_time,( 3959 * acos ( cos ( radians('$latitude') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$longitude') ) + sin ( radians('$latitude') ) * sin( radians( latitude ) ) ) ) AS distance  FROM gpsdata where pos_date='$curdate' and emp_id='$emp_id' HAVING distance < 0.124274 ORDER BY pos_time ASC")->result();
	 ...
	 {
	 	// $curdate=date("Y-m-d",strtotime($formdata['form_date']));
    ...
    public function get_shift_employee_list()
	{
	    ...
	    if(!empty($finalids))
	    {
	    	return $this->db->query(" SELECT * FROM  tbl_admin_login WHERE user_type='E' and  id NOT IN($finalids) ")->result();
	    }
	    else
	    {
	    	return $this->db->query(" SELECT * FROM  tbl_admin_login WHERE user_type='E'  ")->result();
	    }
	...
	// ------------------------------------- date range report ---------------------------------------------
	 ...
	 {
	 	// $form_date=date("Y-m-d",strtotime($formdata['start_date']));
	 	// $to_date=date("Y-m-d",strtotime($formdata['end_date']));
		...
		$to_date = date("Y-m-d", strtotime($end_date1));
	 	...
	 	// echo "SELECT * from gpsdata WHERE pos_date>='$form_date' AND pos_date>='$form_date'<='$to_date' AND IMEI='$imei'";DATE(completed_date)>='$start_date' and DATE(completed_date)<='$end_date' 
