...
class Customer_model extends CI_Model {
    	...
    	public function __construct() 
        {
         ...
         {
           return $this->db->query("SELECT * FROM `tbl_customer` WHERE `delete_status`='1'");
         }

         public function type_list()
         {
           return $this->db->query("SELECT * FROM `tbl_type` WHERE status='1'")->result();
         }
         ...
         {
           return $this->db->query("SELECT * FROM `tbl_location` WHERE status='1'")->result();
         }
         public function business_list()
         {
           return $this->db->query("SELECT * FROM `tbl_business` WHERE status='1'")->result();
         }
         public function group_list()
         {
           return $this->db->query("SELECT * FROM `tbl_group` WHERE status='1'")->result();
         }
            ...
            if($this->session->user_type=='SA')
            {
                $data = $this->db->query("SELECT * FROM `tbl_user_query` ORDER BY query_id DESC");
                 ...
                 foreach ($data->result() as $value)
                 {
                    ...
                    $data3 = $this->db->query("SELECT count(schedule_id) as count FROM `tbl_schedule` WHERE `issue_id`='$query_id'");
