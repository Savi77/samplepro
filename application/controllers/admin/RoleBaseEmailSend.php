...
class RoleBaseEmailSend extends CI_Controller 
{
    public function Yearly()
    {
        $get_list_montly = $this->db->select('*')->from('tbl_userwise_role_report')->where('frequency','yearly')->get()->result();
        
       
        foreach($get_list_montly as $value)
        {
            ...
            $start_date1 = date('Y-m-d', strtotime($get_details->start_date));
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                    <td align="center">
                                                                        <div class="text-center" >
                                                                            <img src="https://buroforce.progfeel.co.in/app-assets/image/task-1.png" alt="img" style="max-width: 70px;width: 100%;height:auto">
                                                                            <h3 style="margin-bottom: 0px;font-size: 32px;font-weight: 400;color: #fff;margin-top: 10px;">Yearly Task Assignment</h3>
                                                                            <h3 style="margin-bottom: 0px;font-size: 32px;font-weight: 400;color: #fff;margin-top: 10px;">'.date('d F, Y', strtotime($active_period_start_date)). ' To ' .date('d F, Y', strtotime($active_period_end_date)).'</h3>
                                                                        </div>
                                                                    </td>
                                                                </tr>
    ...
    public function abc()
	{
        $get_list_yearly = $this->db->select('*')->from('tbl_frequency_type')->where('frequency_type','monthly')->get()->result();
        echo "<pre>";
        print ($get_list_yearly);
        die;
        foreach($get_list_montly as $value)
        {
            $frequency_id = $value->frequency_id;
            ...
            if($start_date == $currentDate && $currentDate <= $end_date)
            {
                ...
                // {
                    if($report == 'Product Lead')
                    {
                        ...
                        $active_period_start_date = $financialYearStart .'04-01';
                        $active_period_end_date = $financialYearEnd . '-'.$currentMonth. '-31';
