...
class Target extends CI_Controller 
{
	...
	public function index()
	{
		if(isset($_SESSION['id']))
		{
			...
			// var_dump($data['target_list']);exit;
			...
			$permission_array=array('user_id'=>$_SESSION['id'],'module_id'=>1,'feature_id'=>8);
			$data['user_permission']=$this->Dashboard_model->get_user_permission
			($permission_array);

			$data['type'] = 'd_link';
			$data['page_name_link'] = 'admin/Master/View_master';
			$data['page_name_1'] = 'Master';
			$data['page_name_2'] = 'Target List';
			$data['sidebar']=array('menu' =>"Master");

			// $this->load->view('Admin/target_list_view',$data);
			$this->load->view('Admin/new_target_list_view',$data);
		}
		else
		{
	...
	public function getemployee_list()
	{
		...
		if(isset($_SESSION['id']))
		{
			...
			// $data['emp_list'] = $this->Target_model->getallemployeefortargetliist($start_date,$end_date,$targettype_id);
	...
	public function getallemployeefortargetliist()
	{
        $start_date = $this->input->post('start_date');
        $end_date = $this->input->post('end_date');
        $targettype_id = $this->input->post('targettype_id');
	...
	public function add_target()
	{
		if(isset($_SESSION['id']))
		{
			...
			$targettype_id = $this->input->post('targettype_id');
			$name_values = $this->input->post('selected_emp');
				...
				$count1 = count(array_filter($taget_value));
	       		...
	       		if ($count!='0' && $count1 !='0') 
	       		{
					...
					// die;
	       			if ($count==$count1) 
		       		{
		       			$this->Target_model->add_target($target_period,$start_date,$end_date,$targettype_id,$name_values,$res,$uom_id,$recurring_cnt);
		       			echo "1";
		       		}
		       		else
		       		{
	...
	public function edit_target()
	{
		if(isset($_SESSION['id']))
		{
			...
			$tr_auto_id = $_REQUEST['tr_auto_id'];
			$data['edit_target']=$this->Target_model->edit_target($targetid);
			$data['target_type']=$this->Target_model->target_type_list();
			$data['selected_employee_list']=$this->Target_model->employee_list($tr_auto_id);
			...
			$data['all_employee_list']=$this->Target_model->getemployee_list($start_date,$end_date,$targetid,$tr_auto_id);
			$data['get_uom']=$this->Target_model->get_uom($targettype_id);
		...
		if(isset($_SESSION['id']))
		{
			...
			{  
	       		if ($count!='0' && $count1 !='0') 
	       		{
	       			if ($count==$count1) 
		       		{
		       			$this->Target_model->edit_add_target($start_date,$end_date,$targettype_id,$name_values,$res,$tr_auto_id,$date_created,$target_period,$recurring_cnt);
		       			echo "1";
		       		}
		       		else
		       		{
	...
	public function archive_target()
	{
		if(isset($_SESSION['id']))
		{
			...
			$data['page_name_link'] = 'admin/Master/View_master';
			$data['page_name_1'] = 'Master';
			$data['page_name_2'] = 'Archive Target List';
			$data['sidebar']=array('menu' =>"Master");

			// $this->load->view('Admin/archive_target_view',$data);
			$this->load->view('Admin/new_archive_target_view',$data);
		}
		else
		{
			redirect('admin/Login');
...
//================= Target type section ========================================
	...
	public function target_type()
	{
		if(isset($_SESSION['id']))
		{
			...
			$data['page_name_link'] = 'admin/Master/View_master';
			$data['page_name_1'] = 'Master';
			$data['page_name_2'] = 'Target Type List';
			$data['sidebar']=array('menu' =>"Master");

			// $this->load->view('Admin/target_type_view',$data);
			$this->load->view('Admin/new_target_type_view',$data);
		}
		else
		{
			redirect('admin/Login');
