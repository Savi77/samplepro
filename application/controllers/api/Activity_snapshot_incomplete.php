...
class Activity_snapshot_incomplete extends REST_Controller 
{
    ...
    public function index_post()
    {
        ...
        if ($user_type == 'C') 
        {
            ...
            for($i=0;$i<COUNT($data2);$i++)
            {
                $f_data = array(
                    ...
                    "type"=> $data2[$i]->type,
                    "assign_date"=> date("d M Y", strtotime($data2[$i]->assign_date)),
        ...
        else if($user_type == 'E')
        {
            ...
            //     $completed = $final_array;
			// }
            $data = $this->db->query(" SELECT * FROM `tbl_user_query`  WHERE org_id='$org_id' and status='in_complete'  and assign_to='$user_id' ORDER BY query_id DESC  ");
        ...
        if(!empty($incompleted))
        {
            $responce = array(
                'status'=>200,
                'msg' =>'Activity Snapshot In-Completed Task Fetched Successfully',
                'data' => $incompleted
                );
        }
        else
