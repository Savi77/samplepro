              ...
              <div class="panel-body" style="padding:0px;">
                <div class="tabbable">
                  <ul class="nav nav-tabs nav-tabs-bottom" style="background-color:#009FDF;">
                    <li class="active" style="font-size: 18px;"><a href="#right-icon-tab1" data-toggle="tab">FAQ List<i class="icon-menu7 position-right"></i></a></li>
                    <li><a href="#right-icon-tab2" data-toggle="tab" style="font-size: 18px;">FAQ Description</i></a></li>
                              ...
                              <tr>
                                ...
                                <td class="hidden"><?= $row->title ?></td>
                                 <td><?php if($row->status==1){ ?>
                                     <a data-popup="tooltip" title="" data-placement="bottom"  data-original-title="Click for Inactive"  id="<?= $row->faq_id?>" onclick="deactivate(this.id)"><span class="label label-success">Active</span></a>
                                   <?php } else {?>

                                   <a  data-popup="tooltip" title="" data-placement="bottom" data-original-title="Click for Activate"  id="<?= $row->faq_id?>"   onclick="activate(this.id)"><span class="label label-danger">Inactive</span></a>
                                    <?php } ?>
                                </td>
                                <td>
                                   <ul class="icons-list">
                                      <li><a  onclick="edit_interest(id)" id="<?= $row->faq_id; ?>"><span class="label bg-success" style="line-height: 20px;"><i class="glyphicon glyphicon-edit" style="font-size: 12px;" data-toggle="tooltip" title="Edit FAQ" data-placement="bottom"></i></span></a></li>
                                      <li><a onclick="del_interest(id)" id="<?= $row->faq_id; ?>"><span class="label bg-danger" style="line-height: 20px;"><i class="glyphicon glyphicon-trash" style="font-size: 12px;" data-toggle="tooltip" title="Delete FAQ" data-placement="bottom"></i></span></a></li>
                                    </ul>
                                </td>
                              </tr>
                            <?php $count++; } ?>
                                      ...
                                      <div class="col-lg-10">
                                        <div class="media no-margin-top">
                                          <div class="media-left">
                                            <?php if ($get_data[0]->section_image == '') { ?>
                                              <a href="#"><img src="<?= base_url() ?>assets/admin/assets/images/placeholder.jpg" style="width: 58px; height: 58px;" class="img-rounded" alt="" id="home_img_feature"></a>    
                                            <?php }else { ?>
                                              <a href="#"><img src="<?= base_url() ?>assets/admin/assets/images/section_image/<?= $get_data[0]->section_image; ?>" style="width: 58px; height: 58px;" class="img-rounded" alt="" id="home_img_feature"></a>
                                            <?php } ?>
                                            
                                          </div>

                                          <div class="media-body">
                                            <input type="file" class="file-styled" id="imgInpfeature" name="fileupFeature">
    ...
    function readURL(input) {
          if (input.files && input.files[0]) {
              ...
              reader.onload = function (e) {
                  $('#home_img_feature').attr('src', e.target.result);
              }
...
<script type="text/javascript">
        ...
        $('#imgInpfeature').bind('change', function() {
        var ext = $('#imgInpfeature').val().split('.').pop().toLowerCase();
                   ...
                   if (e.isDefaultPrevented())
                    ...
                    {   
                        $.ajax({
                           ...
                           url: "<?php echo base_url('BA/Faq/add');?>",
                                  type: "POST",
        ...
        notice.get().on('pnotify.confirm', function()
         {
           ...
           // alert(datastring);
             $.ajax({
              type: "post",
              url: "<?php echo base_url('BA/Faq/deactivate'); ?>",
              cache: false,    
              data: datastring,
              success: function(data)
              {    
        ...
        notice.get().on('pnotify.confirm', function()
         {
           ...
           // alert(datastring);
             $.ajax({
              type: "post",
              url: "<?php echo base_url('BA/Faq/activate'); ?>",
              cache: false,    
              data: datastring,
              success: function(data)
              {    
            ...
            {   
              // alert();
                $.ajax({
                         ...
                         url: "<?php echo base_url('BA/Faq/Update_header');?>",
                                type: "POST",
                                data:  new FormData(this),
                                contentType: false,
                                cache: false,
        ...
        notice.get().on('pnotify.confirm', function()
         {
           ...
           // alert(datastring);
             $.ajax({
              type: "post",
              url: "<?php echo base_url('BA/Faq/delete'); ?>",
              cache: false,    
              data: datastring,
              success: function(data)
              {    
...
<script>
    function edit_interest(id)
    {
      ...
      //alert(datastring);
       $.ajax({
        type: "post",
        url: "<?php echo base_url('BA/Faq/edit'); ?>",
        cache: false,    
        data: datastring,
        success: function(data)
        {    
