                      ...
                      <div class="tabbable">
                        <ul class="nav nav-tabs nav-tabs-bottom" style="background-color:#00619F;">
                          <li class="active" style="font-size: 18px;"><a href="#right-icon-tab1" data-toggle="tab">
                            <i class="icon-make-group position-right" style="zoom:1.4;"></i> &nbsp; Feature List</a></li>
                        </ul>
                                          ...
                                          <?php 
                                            ...
                                            ?>
                                              <tr>
                                                ...
                                                <td class="hidden"></td>
                                                <td style="width: 10%;"><?= $row['component_title']; ?></td>
                                                <td style="width: 70%;"><?= $row['privilege']; ?></td>                                                
                                                <td class="hidden"></td>
                                                <td class="hidden"></td>
                                                <td  style="width: 70%;">
                                                    <ul class="icons-list">
                                                        <li><a data-toggle="modal" onclick="EditDetails(id)" id="<?= $row['component_id']; ?>">
                                                        ...
                                                        <li><a data-toggle="modal" onclick="DeleteDetails(id)" id="<?= $row['component_id']; ?>"><span class="label bg-danger" style="line-height: 20px;"><i class="glyphicon glyphicon-trash" style="font-size: 12px;" data-toggle="tooltip" title="Delete" data-placement="bottom"></i></span></a></li>
                                                    </ul>
                                                </td>
                                              </tr>
                                          <?php $count++; } ?>
                              ...
                              <div class="col-md-12"> 
                                <div class="form-group">
                                   <label>Title : <sup style="color: red">*</sup></label>
                                    <input type="text" class="form-control" name="component_title" autocomplete="off">
                                </div>
                              </div>          
                          </div>
                        </fieldset>  
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="multi-select-full">
                                    <select class="multiselect-select-all" multiple="multiple" name="Privilegeids[]">
                                        <?php   
                                          foreach ($privilege_list as $res) 
                                          {
                                        ?>
                                        <option value="<?= $res->privilege_id;?>"><?= $res->privilege;?></option>
                                       <?php } ?> 
                                    </select>
                                  </div>
                                </div>
           ...
           {  
             ...
             if (e.isDefaultPrevented())
              ...
              else
              {
                  ...
                  $.ajax({
                    url: "<?php echo base_url('BA/FeatureList/AddDetails'); ?>",
                    type: "POST",
                    ...
                    success: function(data)
                      {
                        ...
                        // alert(data);
                         $(function()
                         {
                           ...
                           setTimeout(function()
                             {
                                 window.location="<?php echo base_url('BA/FeatureList');?>";
                             }, 1000);
                      },
                      error: function() 
                      {
...
<script>
function EditDetails(component_id)
{
    $(window).scrollTop(0);
    var datastring = 'component_id='+component_id;
    //alert(datastring);
     $.ajax({
      type: "post",
      url: "<?php echo base_url('BA/FeatureList/EditDetails'); ?>",
      cache: false,    
      data: datastring,
      success: function(data)
      {    
...
<script type="text/javascript">
 function DeleteDetails(component_id)
 {
        ...
        notice.get().on('pnotify.confirm', function()
         {
             var datastring='component_id='+component_id;
              $.ajax({
              url: '<?php echo base_url('BA/FeatureList/DeleteDetails') ?>',
              type: "POST",
              data:  datastring,
              success: function(data)
                {
