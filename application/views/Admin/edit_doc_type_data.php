...
<?php foreach ($edit_doc_type_data->result() as $value) {  ?>
    <form class="form-horizontal" id="EditData" method="POST">
        <input type="hidden" class="form-control" id="doc_id123" name="doc_id123" value="<?= $value->id ?>">

        <div class="form-group">
            <label class="control-label col-sm-12" for="email">Document Name <span style="color: red;">*</span></label>
            <div class="col-sm-12 ">
                <input type="text" class="form-control" id="edit_doc_name" name="edit_doc_name" placeholder="Enter Document Name" onkeyup="chk_doc_type_edit()" value="<?=$value->doc_name;?>">
                <small id="error_doc_type123" style="color: #f00 !important;"></small>
            </div>
        </div>
        <div class="form-group" style="margin-bottom:0;">
            <div class="col-sm-offset-3 col-sm-12" style="text-align:right;">
                <button type="submit" class="btn btn-primary pull-right" id="doc_type_sub_btn123">Update <i class="icon-arrow-right14 position-right"></i></button>
            </div>
        </div>
    </form>
<?php } ?>
    ...
    $(document).ready(function(e) {
        $("#EditData").on('submit', (function(e) {
            ...
            if (e.isDefaultPrevented()) {
                ...
                $.ajax({
                    url: "<?php echo base_url('admin/Master/updateDocType'); ?>",
                    type: "POST",
...
<div class="modal fade" id="UpdatesuccessModal" tabindex="-1" aria-labelledby="UpdatesuccessModalLabel" data-keyboard="false" data-backdrop="static" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="width:400px;">
        ...
        <div class="modal-body" style="font-size: 18px;text-align: center;">Document Type Updated Successfully</div>
...
<script>
    function chk_doc_type_edit()
    {
        
        doc_name = $('#edit_doc_name').val();
        doc_name_id = $('#doc_id123').val();
        
        if (doc_name == '') 
        {
            $('#error_doc_type123').empty();
        ...
        {
            $.ajax({
                url: "<?php echo base_url('admin/Master/chk_doc_type_edit'); ?>",
                dataType: "html",
                type: "POST",
                data: {doc_name: doc_name,doc_name_id: doc_name_id},
                success: function(data) 
                {
                    if (data == 1) 
                    {
                        $('#error_doc_type123').empty();
                        $('#error_doc_type123').css('display','block');
                        $('#error_doc_type123').html('Please add another document name , this document name is already created.');
                        $("#doc_type_sub_btn123").attr('disabled', true);
                        // $().(disabled="disabled")
                    }   
                    else 
                    {
                        $('#error_doc_type123').hide();
                    }
                }
            });
        }
