...
<style>
   ...
   .popover-body{
    ...
    tr.bg {
     ...
     #MyStageListTable_wrapper button.dt-button.buttons-columnVisibility:nth-last-child(1),#MyStageListTable_wrapper button.dt-button.buttons-columnVisibility:nth-last-child(2),#MyStageListTable_wrapper button.dt-button.buttons-columnVisibility:nth-last-child(3){
        display: none;
    }
    .datatable-header {
        padding-top: 1.25rem !important;
    }
    #MyStageListTable th:first-child{
    ...
    <div class="col-lg-12 p-0">
        <div class="card">
            <div class="card-header header-elements-sm-inline py-sm-0 blue-color-card ">	
                <span class="span-py-10 white-text">Stage List</span>
                <a <?= $AddClassP ?> data-toggle="modal" data-target="#StagePopup" class="btn btn-link btn-float has-text"><i class="icon-file-plus text-primary"><span class="left-padding">Add</span></i></a>
            </div>

            <!-- datatable-button-flash-basic -->
            <table class="table table-striped" id="MyStageListTable">
                <thead >
                    <tr>
                        <th>Sr No</th>
                        <th>Stage Title</th>
                ...
                <tbody>
                    ...
                    foreach ($get_stagedata->result() as $row) {
                        if ($count % 2 == 0) {
                        ...
                        <tr <?= $bg_color_class ?>>
                            ...
                            <td <?= $StatusClass; ?>><?php if ($row->status == 1) { ?>
                                <div style="width:150px;">
                                    <button type="button" class="green-btn" data-popup="tooltip" title="Click for Inactive" data-placement="bottom" data-original-title="Click for Inactive" id="<?= $row->stage_id ?>" onclick="deactivate(this)" data-id="<?= $row->stage_id ?>"> Active </button>
                                    <?php } else { ?>
                                        <button type="button" class="red-btn" data-popup="tooltip" title="Click for Activate" data-placement="bottom" data-original-title="Click for Activate" id="<?= $row->stage_id ?>" onclick="activate(this)" data-id="<?= $row->stage_id ?>"> Inactive </button>
                                    <?php } ?>
                                </div>
                            </td>
                            ...
                            <!-- <td class="text-center">
                                <div class="row">
                                    <a class="cursor-pointer" data-toggle="modal" onclick="edit_stage(id)" id="<?= $row->stage_id; ?>" data-popup="tooltip" title="Edit Stage" data-placement="bottom" data-original-title="Edit Stage" <?= $EditClass; ?>><i class="fa fa-edit"></i></a>
                                    <a <?= $DeleteClass; ?> class="cursor-pointer" data-toggle="modal" onclick="DeleteList(this)" data-id="<?= $row->stage_id ?>" id="<?= $row->stage_id; ?>" data-popup="tooltip" title="Delete Stage" data-placement="bottom" data-original-title="Delete Stage"><i class="fa fa-trash"></i></a>
                                        ...
                                        <li>  
                                            <div>
                                                ...
                                                <div class="my-popover-content" style="display:none">
                                                    <ul>
                                                        <li>
                                                            <a data-toggle="modal" onclick="edit_stage(id)" id="<?= $row->stage_id; ?>" style="color:#333333;">
                                                                <span class="status-mark position-left dot dot-green"></span> Edit Stage  
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a data-toggle="modal" onclick="DeleteList(this)" data-id="<?= $row->stage_id ?>" id="<?= $row->stage_id; ?>" style="color:#333333;">
                                                                <span class="status-mark position-left dot dot-red"></span> Delete Stage  
                                                            </a>
                                                        </li>
                                                    </ul>
                                                </div>
...
<div id="StagePopup" class="modal fade" data-backdrop="static" tabindex="-1" role="dialog">
...
<div id="modal_default1" class="modal fade" data-keyboard="false" data-backdrop="static" tabindex="-1" aria-modal="true" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="card-header header-elements-sm-inline py-sm-0 blue-color-card">
                <h6 class="card-title py-sm-4 card-headings">Edit Stage List</h6>
...
<script>
    function del_stage(id) {

        var notice = new PNotify({
            title: 'Confirmation',
            text: '<p>Are you sure you want to delete this Stage?</p>',
            hide: false,
            type: 'warning',
            confirm: {
                confirm: true,
        ...
        notice.get().on('pnotify.confirm', function() {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('admin/Leads/delete_stage'); ?>",
                cache: false,
                data: datastring,
                success: function(data) {
                    //alert(data);
...
<script>
    function edit_stage(id) {
        ...
        $.ajax({
            type: "post",
            url: "<?php echo base_url('admin/Leads/edit_stage'); ?>",
            cache: false,
            data: datastring,
            success: function(data) {
                // alert(data);
...
<script>
    function deactivate(id) {
        ...
        var notice = new PNotify({
            title: 'Confirmation',
            text: '<p>Are you sure you want to Inactive this Stage?</p>',
            hide: false,
            type: 'warning',
            confirm: {
                confirm: true,
        ...
        notice.get().on('pnotify.confirm', function() {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('admin/Leads/deactivate1'); ?>",
                cache: false,
                data: datastring,
                success: function(data) {
                    // alert(data);
...
<script>
    function activate(id) {
        ...
        var notice = new PNotify({
            title: 'Confirmation',
            text: '<p>Are you sure you want to activate this Stage?</p>',
            hide: false,
            type: 'warning',
            confirm: {
                confirm: true,
        ...
        notice.get().on('pnotify.confirm', function() {
            ...
            $.ajax({
                type: "post",
                url: "<?php echo base_url('admin/Leads/activate1'); ?>",
                cache: false,
                data: datastring,
                success: function(data) {
                    //alert(data);
...
<div class="modal fade" id="deleteconfirmationModal" tabindex="-1" aria-labelledby="confirmationModalLabel" data-keyboard="false" data-backdrop="static" style=" padding-right: 15px;" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered" style="width:400px">
        <div class="modal-content" style="padding: 30px">
            ...
            <div class="modal-body" style="font-size: 18px; text-align: center;">Shall we proceed with deleting this Stage?</div>
                <div class="modal-footer" style="justify-content: center;">
                    <form class="form-horizontal" id="deletstageForm">
...
<div class="modal fade" id="deactivateconfirmationModal" tabindex="-1" aria-labelledby="deactivateconfirmationModalLabel" data-keyboard="false" data-backdrop="static" style=" padding-right: 15px;" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered" style="width:400px">
        <div class="modal-content" style="padding: 30px">
            ...
            <div class="modal-body" style="font-size: 18px; text-align: center;">Shall we proceed with inactivating this Stage?</div>
                <div class="modal-footer" style="justify-content: center;">
...
<div class="modal fade" id="activateconfirmationModal" tabindex="-1" aria-labelledby="activateconfirmationModalLabel" data-keyboard="false" data-backdrop="static" style=" padding-right: 15px;" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered" style="width:400px">
        <div class="modal-content" style="padding: 30px">
            ...
            <div class="modal-body" style="font-size: 18px; text-align: center;">Shall we proceed with activating this Stage?</div>
                <div class="modal-footer" style="justify-content: center;">
...
$(document).ready(function(e) 
{
  $("#deletstageForm").on('submit', (function(e) 
  {
    //e.preventDefault();
    if (e.isDefaultPrevented()) 
    {
...
<script>
    $(document).ready(function(){
        var rescheduleTable = $('#MyStageListTable').DataTable( {       
        scrollCollapse: true,
        paging:         true, 
        // order: [[0, 'desc']],
        // fixedColumns: true,
