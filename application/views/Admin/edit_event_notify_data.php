...
<?php foreach ($edit_event_notify->result() as $value) {  ?>
    <form class="form-horizontal" id="EditData" method="POST">
        <input type="hidden" class="form-control" id="event_notify123" name="event_notify123" value="<?= $value->id ?>">

        <div class="form-group">
            <label class="control-label col-sm-12" for="email">Event Notify Name <span style="color: red;">*</span></label>
            <div class="col-sm-12 ">
                <input type="text" class="form-control" id="edit_event_notify_name" name="edit_event_notify_name" placeholder="Enter Event Notify Name" onkeyup="chk_event_notify_edit()" value="<?=$value->event_notify_name;?>">
                <small id="error_event_notify123" style="color: #f00 !important;"></small>
            </div>
        </div>
        <div class="form-group" style="margin-bottom:0;">
            <div class="col-sm-offset-3 col-sm-12" style="text-align:right;">
                <button type="submit" class="btn btn-primary pull-right" id="event_notify_sub_btn123">Update <i class="icon-arrow-right14 position-right"></i></button>
            </div>
        </div>
    </form>
<?php } ?>
    ...
    $(document).ready(function(e) {
        $("#EditData").on('submit', (function(e) {
            ...
            if (e.isDefaultPrevented()) {
                ...
                $.ajax({
                    url: "<?php echo base_url('admin/Master/updateEventNotify'); ?>",
                    type: "POST",
...
<div class="modal fade" id="UpdatesuccessModal" tabindex="-1" aria-labelledby="UpdatesuccessModalLabel" data-keyboard="false" data-backdrop="static" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="width:400px;">
        ...
        <div class="modal-body" style="font-size: 18px;text-align: center;">Event Notify Updated Successfully</div>
...
<script>
    function chk_event_notify_edit()
    {
        
        event_notify_name = $('#edit_event_notify_name').val();
        event_notify_id = $('#event_notify123').val();
        
        if (event_notify_name == '') 
        {
            $('#error_event_notify123').empty();
        ...
        {
            $.ajax({
                url: "<?php echo base_url('admin/Master/chk_event_notify_edit'); ?>",
                dataType: "html",
                type: "POST",
                data: {event_notify_name: event_notify_name,event_notify_id: event_notify_id},
                success: function(data) 
                {
                    if (data == 1) 
                    {
                        $('#error_event_notify123').empty();
                        $('#error_event_notify123').css('display','block');
                        $('#error_event_notify123').html('Please add another event notify name , this event notify name is already created.');
                        $("#event_notify_sub_btn123").attr('disabled', true);
                        // $().(disabled="disabled")
                    }   
                    else 
                    {
                        $('#error_event_notify123').hide();
                    }
                }
            });
        }
