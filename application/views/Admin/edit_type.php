...
?>
  <form class="form-horizontal" id="EditType">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-12">
        <input type="text" class="form-control" id="title123" name="title" value="<?= $value->title ?>" placeholder="Enter Contact Type Title" maxlength="50" onkeyup="chk_contact_type_edit()">
        <input type="hidden" class="form-control" id="title_id" name="title_id" value="<?= $value->type_id ?>">
        <small id="error_contact_type123" style="color: #f00 !important;"></small>
      </div>
    </div>

    <div class="form-group" style="margin-bottom:0;">
      <div class="col-sm-offset-3 col-sm-12" style="text-align: right;">
        <button type="submit" class="btn btn-primary pull-right" id="contact_type_sub_btn123">Update<i class="icon-arrow-right14 position-right"></i></button>
      </div>
    </div>
  </form>
...
<div class="modal fade" id="UpdatesuccessModal" tabindex="-1" aria-labelledby="UpdatesuccessModalLabel" data-keyboard="false" data-backdrop="static" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="width:400px;">
        ...
        <div class="modal-body" style="font-size: 18px;text-align: center;">Contact Type Updated Successfully</div>
...
<script>
    function chk_contact_type_edit()
    {
        title = $('#title123').val();
        ...
        {
            $.ajax({
                url: "<?php echo base_url('admin/Master/chk_contact_type_edit'); ?>",
                dataType: "html",
                type: "POST",
                data: {title: title,title_id: title_id},
                success: function(data) 
                {
                    ...
                    {
                        $('#error_contact_type123').empty();
                        $('#error_contact_type123').css('display','block');
                        $('#error_contact_type123').html('Please add another contact type, this contact type is already created.');
                        $("#contact_type_sub_btn123").attr('disabled', true);
                        // $().(disabled="disabled")
                    }   
                    else 
                    {
                        $('#error_contact_type123').hide();
                    }
                }
            });
        }
